...
const defaultSettings: Settings = {
  ...
  ttsEnglish: { voiceName: "", rate: 1.0, pitch: 1 },
  ttsCantonese: { voiceName: "", rate: 0.8, pitch: 1 },
  ttsMandarin: { voiceName: "", rate: 0.8, pitch: 1 },
}
...
const tempSettings = reactive<TempSettings>({ ttsCantonese: null, ttsMandarin: null, ttsEnglish: null})
...
function loadVoices() {
	...
	voices.forEach(function(voice) {
			...
			listVoiceOptions.value.push({ value: voice.name, text: name })
...
export const useSettingsStore = defineStore('settings', () => {
  return { settings, tempSettings, listVoiceOptions }
})
